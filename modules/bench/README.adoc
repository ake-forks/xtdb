...
Postgres source::
`docker-compose up -d postgres`, then `./gradlew pg-source-tx-overhead -PdocCount=100000 -PbatchSizes=1000,100,10,1`
+
...
The node config (registering the docker-compose Postgres as the `pg` remote) defaults to `modules/bench/config/pg-source.yaml` - override with `-PconfigFile=...`, keeping it in step with `-PpgUrl`, which is where the benchmark's own writes go.
The benchmark opens and closes its own nodes, overriding that config's log and storage with a per-batch-size local directory - the source resumes from a token that has to survive the restart.
