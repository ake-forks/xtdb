...
+
The same transaction-size sweep as IngestTsOverhead, but through CDC: each batch size writes `docCount` rows into Postgres, and a `!Postgres` source mirrors them into an attached XTDB database.
`pg-ingest-batch-N` is the write into Postgres and `xt-mirror-batch-N` is how far behind the mirror still was when that write stopped; per-transaction latency comes from the source's own `commit_lag_seconds` summary, logged at the end of each mirror stage.
...
Each run takes over the `batched_*` and `sentinel` tables and the `xtdb_bench_pg_source` publication and slot in that database, dropping and recreating them before it starts.
